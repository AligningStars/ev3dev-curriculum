...
#     300 degrees / second  -->  traveled 34.5 inches  -->  3.45 inches /
# second
...
#     900 degrees / second  -->  traveled 42 inches  -->  8.4 inches / second
...
def main():
    ...
    while time_s != 0:
        ...
        times_sp = dis_in / motor_inps
        if motor_dps == 0 or dis_in == 0:
            break

        left_motor.run_forever(speed_sp=motor_dps, time_sp=times_sp)
        right_motor.run_forever(speed_sp=motor_dps, time_sp=times_sp)
        time.sleep(time_s)
        left_motor.stop()
